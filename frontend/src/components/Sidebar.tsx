...
import { MessageSquare, Plus, Trash2, FileText, Sparkles, X, Eraser, Menu } from 'lucide-react'
...
export default function Sidebar({ sessions, activeId, onNew, onOpen, onDelete, onClear }: Props) {
  ...
  return (
    <>
      ...
      <button
        ...
        className="fixed left-4 top-3 z-50 flex h-9 w-9 items-center justify-center rounded-xl
          border border-edge bg-panel/80 backdrop-blur-xl text-slate-400
          transition-colors hover:text-cyan-400 lg:hidden"
      >
      ...
      <aside
        className={`glass-strong z-40 flex h-full w-72 shrink-0 flex-col border-r
          transition-transform duration-200
          ${mobileOpen ? 'fixed inset-y-0 left-0 translate-x-0' : '-translate-x-full lg:translate-x-0 lg:relative'}`}
      >
        {/* brand */}
        <div className="flex items-center gap-3 px-5 pb-4 pt-5">
          ...
          <button
            onClick={() => setMobileOpen(false)}
            className="rounded-lg p-1.5 text-slate-500 hover:text-slate-300 lg:hidden"
          >
            <X className="h-5 w-5" />
          </button>
        </div>
          ...
          {sessions.length === 0 ? (
            ...
            <ul className="space-y-1">
              {sessions.map((s) => (
                <li key={s.session_id}>
                  <button
                    onClick={() => { onOpen(s.session_id); setMobileOpen(false) }}
                  ...
                  >
                    ...
                    <span className="flex shrink-0 gap-0.5 opacity-0 transition-all group-hover:opacity-100">
                      <span
                        role="button"
                        tabIndex={0}
                        onClick={(e) => {
                          e.stopPropagation()
                          onClear(s.session_id)
                        }}
                        onKeyDown={(e) => e.key === 'Enter' && onClear(s.session_id)}
                        title="Clear history"
                        className="rounded-md p-1 text-slate-500 hover:text-amber-400"
                      >
                      ...
                      <span
                        ...
                        className="rounded-md p-1 text-slate-500 hover:text-red-400"
                      >
